...
// Soft Spoon (pre-theDAO art network) difficulty parameters. The one-time reset at the
// Soft Spoon block cannot be expressed via chain config, so it is applied here and gated by
